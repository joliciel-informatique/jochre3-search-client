...
<script setup lang="ts">
    ...
    const authorList = defineModel<Array<{ label : string, count : number }>>('authorList', { default : []})
    ...
    const delAuthor = (value: string) => {
        authorList.value = authorList.value.filter((author: { label: string, count : number }) => author.label !== value)
    }
    ...
    if (authorText.value.length > 0) {
        ...
        .then((response) => response.json().then((result) => { 
            authorDropdownItems.value = result.bins.filter((author: { label : string, count : number }) => !authorList.value.map(author => author.label).includes(author.label))
            // console.log(exclude.value)
            if (exclude.value) {
                authorDropdownItems.value = authorDropdownItems.value.filter((author: { label : string, count : number }) => author.label !== exclude.value)
            }
        }))
    } else {
        authorDropdownItems.value = []
