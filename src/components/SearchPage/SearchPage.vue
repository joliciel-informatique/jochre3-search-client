...
<script setup lang="ts">
  ...
  const defineSearchParams = () => Object.assign(
    {},
...
const search = (facet: string | undefined = undefined) => {
  ...
  if (hasSearch) {
      ...
      if (docRefs.value) docRefs.value.split(/\W+/).forEach(docRef => params.append('doc-refs', docRef))
      ...
      .then(({results, totalCount}) => {
        authors.value = results.map((result: SearchResult) => result.metadata.author !== '' ? result.metadata.author : '').filter((author: string)=>author)
