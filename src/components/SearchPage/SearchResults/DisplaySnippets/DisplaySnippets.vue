  ...
  <div class="snippetsColumn column" tabindex="-1">
    ...
    <div v-if="!isLoading">
      ...
      <div class="snippets-on-desktop is-hidden-touch">
        <ul>
          <li v-for="(result, bookIndex) of searchResults" :key="sha1(result)" :bookIndex>
            <div v-if="!displayPerBook">
              <ul v-show="selectedEntry?.docRef === result.docRef">
                <li v-for="(snippet, index) in result.snippets" :key="sha1(snippet)">
                  <SingleSnippet
            ...
            <div v-else>
              <ul>
                <li v-for="(snippet, index) in result.snippets" :key="sha1(snippet)" :bookIndex>
                  <SingleSnippet
                    v-model:image-modal="imageModal"
                    v-model:word-modal="wordModal"
                    v-model:notification="notification"
